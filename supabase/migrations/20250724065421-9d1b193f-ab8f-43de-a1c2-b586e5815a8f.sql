-- Update the achievement progress function to handle new achievement types
...
BEGIN
  ...
  -- For subjects_mastered achievements, count distinct subjects
  IF p_type = 'subjects_mastered' THEN
    SELECT COUNT(DISTINCT category) INTO subjects_count
    FROM public.learning_sessions 
    WHERE user_id = p_user_id AND correct_answers > 0;
    
    RAISE LOG 'Subjects mastered by user: %', subjects_count;
  END IF;
